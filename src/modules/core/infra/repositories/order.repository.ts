    ...
    async findByOrganizationId(organizationId: string): Promise<Order[]> {
        try {
            const entities = await this.orderRepository
                ...
                .where(':organizationId = ANY(order.organizationIds)', {
                    organizationId,
                })
            ...
            throw new RepositoryException(
                `Failed to find orders by organization id: ${error.message}`,
            );
        }
    }
    ...
    async findByIdAndOrganizationId(
        ...
        try {
            const entity = await this.orderRepository
                ...
                .andWhere(':organizationId = ANY(order.organizationIds)', {
                    organizationId,
                })
                .getOne();
            ...
            throw new RepositoryException(
                `Failed to find order by id and organization id: ${error.message}`,
            );
        }
    }
}
